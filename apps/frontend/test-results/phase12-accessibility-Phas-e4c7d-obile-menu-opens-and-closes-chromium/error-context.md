...
- Name: phase12-accessibility.spec.ts >> Phase 12 — Accessibility & Responsive >> Mobile menu >> Mobile menu opens and closes
- Location: e2e\phase12-accessibility.spec.ts:158:9
...
Locator: getByRole('link', { name: /صفحه اصلی/i })
Expected: visible
Error: strict mode violation: getByRole('link', { name: /صفحه اصلی/i }) resolved to 2 elements:
    1) <a href="/" aria-label="LEGALIR — صفحه اصلی" class="flex items-center gap-2.5 hover:opacity-80 transition-opacity shrink-0">…</a> aka getByRole('link', { name: 'LEGALIR — صفحه اصلی' })
    2) <a href="/" class="block px-4 py-3.5 rounded-medium text-body-1 transition-colors mb-1 bg-primary-50 text-primary-700 font-medium">صفحه اصلی</a> aka getByRole('link', { name: 'صفحه اصلی', exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByRole('link', { name: /صفحه اصلی/i })
